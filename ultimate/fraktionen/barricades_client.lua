...
function sandbag_func ()
	...
	if vioClientGetElementData ( "job" ) == "pionier" and getElementData ( lp, "fraktion" ) == 8 then
		if not placingBarricade then
			if barricadeTimer <= getTickCount() then
				vioClientSetElementData ( "object", 2060 )
				startObjectDrop ()
				placingBarricade = true
				
				barricadeTimer = getTickCount()+30000
			else
				outputChatBox ( "Du kannst nur alle 30 Sekunden ein Objekt platzieren!", 125, 0, 0 )
			end
		end
	else
...
function barricade_func ()
	...
	if barricadeFactions[getElementData ( lp, "fraktion" )] then
		if not placingBarricade then
			if barricadeTimer <= getTickCount() then
				vioClientSetElementData ( "object", 1422 )
				startObjectDrop ()
				placingBarricade = true
