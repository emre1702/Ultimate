...
local geldbonusfuereroberung = 1000	    				-- Der Bonus (Geld), wenn man ein Gebiet erobert
local geldprodamageimgw = 4     						-- Geld pro Damage
local geldprokillimgw = 200     						-- Geld pro Kill

-- Allgemeine Einstellungen zum Gangwar --
local maxanzahlanattacks = 4   							-- Maximale Anzahl an Attacks pro Tag
local minanzahlangwteilnehmern = 3   					-- Mindestanzahl an Spielern in beiden Frakis für Attack
...
local sofortigerabbruchbeinullverteidigern = false    	-- Ob der GW direkt für Angreifer gewonnen ist, wenn es keine lebenden Verteidiger im GW mehr gibt
...
local gangAttacks = { [2] = maxanzahlanattacks, [3] = maxanzahlanattacks, [7] = maxanzahlanattacks, [9] = maxanzahlanattacks, [12] = maxanzahlanattacks, [13] = maxanzahlanattacks }
local teamBlips = { [2] = {}, [3] = {}, [7] = {}, [9] = {}, [12] = {}, [13] = {} }
...
factionGangAreas = { [2] = 0, [3] = 0, [7] = 0, [9] = 0, [12] = 0, [13] = 0 }
...
function gangPickupHit ( hit )
	...
	if gangAttacks[fraktion] then
		if vioGetElementData ( hit, "rang" ) >= 2 then
...
function gangattack_func ( player )
	if gangAreaAttackbar then
		if not gangAreaUnderPreparation and not gangAreaUnderAttack then
			...
			local attackerfrac = vioGetElementData ( player, "fraktion" )
			if gangAttacks[attackerfrac] and vioGetElementData ( player, "rang" ) >= mindestrangzumattack then
...
function startGangAreaAttack ( )
	...
	for key, thePlayer in pairs ( getElementsWithinColShape ( gangPickup[validID]["colshape"], "player" ) ) do
		if vioGetElementData ( thePlayer, "fraktion" ) == gangData.attackerfrac then
...
function areaEroberungsCheck ( )
	...
	for index, player in pairs ( getElementsWithinColShape ( gangPickup[validID]["tkcolshape"], "player" ) ) do
		if vioGetElementData ( player, "fraktion" ) == gangData.attackerfrac then	
...
function gangwarDefenderGewonnen ()
	...
	for playeritem, tables in pairs ( playerData ) do
		...
		if isElement ( player ) and vioGetElementData ( player, "fraktion" ) == gangData.attackerfrac then
			vioSetElementData ( player, "AnzahlGangwarsVerloren", ( vioGetElementData ( player, "AnzahlGangwarsVerloren" ) or 0 ) + 1 )
		elseif isElement ( player ) then
			vioSetElementData ( player, "AnzahlGangwarsGewonnen", ( vioGetElementData ( player, "AnzahlGangwarsGewonnen" ) or 0 ) + 1 )
		end
	end
...
local function dontLetOthersUseIt ( player )
	if vioGetElementData ( player, "fraktion" ) ~= gangData.ownerfrac then
		cancelEvent()
	end
end
...
function setBurritosBackInGangwar ( player )
	if gangAreaUnderPreparation and vioGetElementData ( player, "fraktion" ) == gangData.attackerfrac then
		local x, y, z = getElementPosition ( gangPickup[validID]["pickup"] )
...
function gangwarPreparation ( player )
	...
	elseif isPedInVehicle ( player ) then
		...
		if seat == 0 then
			...
			if factionVehicles[vioGetElementData ( player, "fraktion")][vehicle] then
				isfracvehicle = true
				spielerSitztImFahrzeug[player] = vehicle
				spielerSitztImFahrzeugSitz[player] = 0
				for seat, occupant in pairs ( getVehicleOccupants ( vehicle ) ) do
					if seat ~= 0 then
						if vioGetElementData ( occupant, "fraktion" ) == vioGetElementData ( player, "fraktion" ) then
							if vioGetElementData ( occupant, "fraktion" ) == gangData.ownerfrac then
							...
							elseif vioGetElementData ( occupant, "fraktion" ) == gangData.attackerfrac then
	...
	if not playerData[pname] then
		...
		vioSetElementData ( player, "AnzahlGangwars", ( vioGetElementData ( player, "AnzahlGangwars" ) or 0 ) + 1 )
	end
	playerData[pname]["imGW"] = true
	playerData[pname]["hatDMGbekommen"] = false
	playerData[pname]["damage"] = 0
	playerData[pname]["kills"] = 0
	playerData[pname]["attacker"] = ( vioGetElementData ( player, "fraktion" ) == gangData.attackerfrac )
...
function gangwarDeathfunc ( attacker )
	...
	if playerData[pname] and playerData[pname]["imGW"] then
		...
		local frac = vioGetElementData(client,"fraktion")
		...
		local isattacker = vioGetElementData ( client, "fraktion" ) == gangData.attackerfrac
		if gangAreaUnderPreparation then
			if isattacker then
				gangData.attacker = gangData.attacker - 1
		...
		elseif gangAreaUnderAttack then
			...
			if isElement ( attacker ) and getElementType ( attacker ) == "player" and attackername and playerData[attackername] and playerData[attackername]["imGW"] and vioGetElementData ( attacker, "fraktion" ) ~= vioGetElementData ( client, "fraktion" ) then
				playerData[attackername]["kills"] = playerData[attackername]["kills"] + 1
				vioSetElementData ( attacker, "GangwarKills", ( vioGetElementData ( attacker, "GangwarKills" ) or 0 ) + 1 )
				triggerClientEvent ( attacker, "rechneKillAn", attacker, playerData[attackername]["kills"] )
				if vioGetElementData ( client, "lasthp" ) then
					triggerClientEvent ( attacker, "rechneDMGAn", attacker, vioGetElementData ( client, "lasthp" ) )
					playerData[attackername]["damage"] = playerData[attackername]["damage"] + vioGetElementData ( client, "lasthp" )
					vioSetElementData ( attacker, "GangwarDamageGemacht", ( vioGetElementData ( attacker, "GangwarDamageGemacht" ) or 0 ) + vioGetElementData ( client, "lasthp" ) )
					vioSetElementData ( client, "GangwarDamageBekommen", ( vioGetElementData ( client, "GangwarDamageBekommen" ) or 0 ) + vioGetElementData ( client, "lasthp" ) )
				end
		...
		vioSetElementData ( client, "GangwarTode", ( vioGetElementData ( client, "GangwarTode" ) or 0 ) + 1 )
	end		
end
...
function gangwarOfflinefunc ( )
	...
	if playerData[pname] then
		...
		if isElement ( teamBlips[vioGetElementData(source,"fraktion")][source] ) then
			destroyElement ( teamBlips[vioGetElementData(source,"fraktion")][source] )
		end
		local isattacker = vioGetElementData ( source, "fraktion" ) == gangData.attackerfrac
		if gangAreaUnderPreparation then
			if isattacker then
				gangData.attacker = gangData.attacker - 1
				gangData.attackeralive = gangData.attackeralive - 1
...
function spawnPlayerAfterGangwar ( player )
	if isElement ( player ) then
		...
		spawnPlayer ( player, vioGetElementData ( player, "spawnpos_x" ), vioGetElementData ( player, "spawnpos_y" ), vioGetElementData ( player, "spawnpos_z" ), (vioGetElementData ( player, "spawnrot_x" ) or 0), skinid, vioGetElementData ( player, "spawnint" ), vioGetElementData ( player, "spawndim" ), team ) 
	end
end
...
function allowAttack ( player, cmd, targetfrac )
	local eigenefrac = vioGetElementData ( player, "fraktion" )
	if gangAttacks[eigenefrac] and vioGetElementData ( player, "rang" ) >= minrangzumattackgeben then
		if targetfrac then
			local fraczahl = nil
			local fracname = ""
...
function gangwarAllowToJoinPluzBattlexChampStinkt ( player, cmd, targetpl )
	if gangAreaUnderAttack then
		if vioGetElementData ( player, "fraktion" ) == gangData.attackerfrac or vioGetElementData ( player, "fraktion" ) == gangData.ownerfrac then
			if vioGetElementData ( player, "rang" ) >= minrangzumjoinerlaubnisgeben then
				if targetpl and findPlayerByName ( targetpl ) then
					local targetpl = findPlayerByName ( targetpl )
					if vioGetElementData ( targetpl, "fraktion" ) == gangData.attackerfrac or vioGetElementData ( targetpl, "fraktion" ) == gangData.ownerfrac then
						if vioGetElementData ( targetpl, "fraktion" ) ~= vioGetElementData ( player, "fraktion" ) then
...
function alleGangwarBefehle (player)
	...
	local adminlvl = vioGetElementData ( player, "adminlvl" )
	if adminlvl >= 3 then
