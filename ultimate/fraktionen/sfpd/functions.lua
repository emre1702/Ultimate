...
function saveDatabaseFile ( name, text )
	...
	if #text <= 1000 then
		...
		if result and result[1] then
			...
			dbExec ( handler, "INSERT INTO state_files ( UID, text, editor, faction ) VALUES (?,?,?,?)", playerUID[name], text, factionRankNames[vioGetElementData ( player, "fraktion" )][vioGetElementData ( player, "rang" )]..getPlayerName ( player ), vioGetElementData ( player, "fraktion" ) )
