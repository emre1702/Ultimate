...
function insertInGang ( pname, id, rank, founder )
	...
	if founder then
		dbExec ( handler, "INSERT INTO gang_members ( UID, Gang, Rang, Founder ) VALUES ( ?, ?, ?, ? )", playerUID[pname], id, rank, 1 )
	else
		dbExec ( handler, "INSERT INTO gang_members ( UID, Gang, Rang ) VALUES ( ?, ?, ? )", playerUID[pname], id, rank )
	end
end
