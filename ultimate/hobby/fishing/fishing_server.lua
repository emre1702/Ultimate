...
function saveFishingValues ( player )
	...
	local string
	if vioGetElementData ( player, "fishingPole" ) then
		string = "1|"
	else
		string = "0|"
	end
	string = string..vioGetElementData ( player, "fishingHooks" ).."|"..vioGetElementData ( player, "fishingWorms" ).."|"
	string = string..vioGetElementData ( player, "fishingFishATyp" )..";"..vioGetElementData ( player, "fishingFishAWeight" ).."|"
...
function fishCought_func ()
	...
	local player = client
	if vioGetElementData ( player, "fishingPole" ) and vioGetElementData ( player, "fishingHooks" ) > 0 and vioGetElementData ( player, "fishingWorms" ) > 0 then
		if getFreeFishSlot ( player ) then
			...
			if math.random ( 1, math.floor ( skill ^ ( 0.5 ) * 10 ) ) >= 5 then
				...
				if fishTyp == 7 then
					vioSetElementData ( player, "money", vioGetElementData ( player, "money" ) + weight )
					infobox ( player, "Du hast einen\nGeldbeutel gefangen!\nEs sind "..weight.." $ drin!", 5000, 0, 125, 0 )
				else
					vioSetElementData ( player, "fishingFish"..slot.."Typ", fishTyp )
					vioSetElementData ( player, "fishingFish"..slot.."Weight", weight )
					infobox ( player, "Du hast etwas\ngefangen:\n"..fishNames[fishTyp], 5000, 0, 200, 0 )
				end
			else
				infobox ( player, "Die Schnur ist\ngerissen!", 5000, 125, 0, 0 )
			end
			vioSetElementData ( player, "fishingHooks", vioGetElementData ( player, "fishingHooks" ) - 1 )
			vioSetElementData ( player, "fishingWorms", vioGetElementData ( player, "fishingWorms" ) - 1 )
		else
...
function getFreeFishSlot ( player )
	...
	if vioGetElementData ( player, "fishingFishATyp" ) == 0 then
		return "A"
	elseif vioGetElementData ( player, "fishingFishBTyp" ) == 0 then
		return "B"
	elseif vioGetElementData ( player, "fishingFishCTyp" ) == 0 then
		return "C"
	end
	return false
end
...
function fishShopBuy_func ( item, amount )
	...
	if item == "pole" then
		if money >= fishingPolePrice then
			if not vioGetElementData ( player, "fishingPole" ) then
				vioSetElementData ( player, "money", money - fishingPolePrice )
				infobox ( player, "Du hast eine\nAngel gekauft!\nDie Angel ist\njetzt in deinem\nInventar.", 5000, 0, 125, 0 )
				vioSetElementData ( player, "fishingPole", true )
			else
		...
		if vioGetElementData ( player, "fishingPole" ) then
			...
			if item == "hooks" then
				local cur = vioGetElementData ( player, "fishingHooks" )
				if money >= fishingHookPrice * amount then
					if cur + amount <= max then
						vioSetElementData ( player, "money", money - fishingHookPrice * amount )
						infobox ( player, "Du hast "..amount.."\nAngelhaken gekauft!", 5000, 0, 125, 0 )
						vioSetElementData ( player, "fishingHooks", amount + cur )
					else
				...
				local cur = vioGetElementData ( player, "fishingWorms" )
				if money >= fishingWormPrice * amount then
					if cur + amount <= max then
						vioSetElementData ( player, "money", money - fishingWormPrice * amount )
						infobox ( player, "Du hast "..amount.."\nWuermer gekauft!", 5000, 0, 125, 0 )
						vioSetElementData ( player, "fishingWorms", amount + cur )
					else
	...
	if getDistanceBetweenPoints3D ( x, y, z, getElementPosition ( player ) ) <= 7.5 then
		if tonumber ( slot ) then
			...
			if slot == 1 or slot == 2 or slot == 3 then
				...
				local fish = vioGetElementData ( player, "fishingFish"..slot.."Typ" )
				if fish then
					local weight = vioGetElementData ( player, "fishingFish"..slot.."Weight" )
					local money = math.floor ( fishPrices[fish] * weight / 1000 )
					
					vioSetElementData ( player, "money", vioGetElementData ( player, "money" ) + money )
					vioSetElementData ( player, "fishingFish"..slot.."Typ", 0 )
					vioSetElementData ( player, "fishingFish"..slot.."Weight", 0 )
