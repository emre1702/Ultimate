...
function joinRCClub ()
	...
	local player = client
	if vioGetElementData ( player, "money" ) >= 250 then
		...
		vioSetElementData ( player, "club", "rc" )
	else
...
function rcVanEnter ( player, seat )
	...
	local veh = source
	local rcID = vioGetElementData ( veh, "rcVehicle" )
	if rcID then
...
function userc ( player )
	...
	if veh then
		if getPedOccupiedVehicleSeat ( player ) > 1 then
			local rcID = vioGetElementData ( veh, "rcVehicle" )
			if rcID then
				if vioGetElementData ( veh, "rcVehicleInUse" ) then
					...
					if getElementModel ( veh ) == 459 then
						...
						vioSetElementData ( player, "RCVanSeat", getPedOccupiedVehicleSeat ( player ) )
						vioSetElementData ( player, "RCVan", veh )
						local dummy = createPed ( getElementModel ( player ), 0, 0, 0 )
						vioSetElementData ( dummy, "player", player )
...
function rcVehicleExited ( veh )
	...
	local van = vioGetElementData ( player, "RCVan" )
	local seat = vioGetElementData ( player, "RCVanSeat" )
...
function rcVehicleDummyWasted ()
	...
	local ped = source
	local player = vioGetElementData ( dummy, "player" )
	destroyElement ( ped )
