...
function VehicleFischerEnter ( player, seat )
	if vioGetElementData ( player, "job" ) ~= "fischer" then
		opticExitVehicle ( player )
		triggerClientEvent ( player, "infobox_start", player, "\n\nDu bist\nkein Fischer!", 5000, 125, 0, 0 )
	elseif vioGetElementData ( player, "job" ) == "fischer" then
		triggerClientEvent ( player, "startfishing", player )
		local t = getRealTime()
		local minutes = t.minute 
		local hour = t.hour
		vioSetElementData ( player, "fishingStartedM", minutes )
		vioSetElementData ( player, "fishingStartedH", hour )
	end
end
	...
	if source == client then
		...
		-- ANTICHEAT --
			if fische >= 1 then
				...
				local einnahmen = math.floor(einnahmen)*20
