...
addEventHandler("onMarkerHit", marker, function(hitElement)
	if(getElementType(hitElement) == "player") then
		...
		if vioGetElementData ( hitElement, "job" ) == "gabelstablerjob" then
...
addEventHandler("onGabelstablerJobStop", getRootElement(), function()
	local thePlayer = source
	if(stabler[thePlayer]) and (vioGetElementData(thePlayer, "job") == "gabelstabler") then
		destroyElement(stabler[thePlayer])
		vioSetElementData(thePlayer, "job", false)
