...
function taxiEnter_func ( player, seat, jacked )
	if seat == 0 then
		if vioGetElementData ( player, "job" ) == "taxifahrer" then
...
function taxiPayCheck_func ()
	...
	if isElement ( player ) and isElement ( driver ) then
		if vioGetElementData ( player, "money" ) >= taxiPricePerInterval then
...
function accept_taxi ( player, cmd, arg )
	if arg == "taxi" then
		local veh = getPedOccupiedVehicle ( player )
		if veh and isVehicleTaxiLightOn ( veh ) and vioGetElementData ( player, "job" ) == "taxifahrer" then
