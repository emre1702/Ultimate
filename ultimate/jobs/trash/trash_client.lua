...
function showStreetCleanGUI_func ()
	...
	if gWindow["cleanJob"] then
		...
		addEventHandler ( "onClientGUIClick", gButton[2], 
			function()
				if vioClientGetElementData ( "streetCleanPoints" ) >= 50 then
					guiSetVisible ( gWindow["cleanJob"], false )
					showCursor ( false )
					triggerServerEvent ( "cancel_gui_server", lp )
		...
		addEventHandler ( "onClientGUIClick", gButton[3], 
			function()
				if vioClientGetElementData ( "streetCleanPoints" ) >= 250 then
					guiSetVisible ( gWindow["cleanJob"], false )
					showCursor ( false )
					triggerServerEvent ( "cancel_gui_server", lp )
	...
	guiSetText ( gLabel[2], tostring ( vioClientGetElementData ( "streetCleanPoints" ) ) )
end
