...
function trashJobIconHit ( player )
	...
	if vioGetElementData ( player, "job" ) == "streetclean" and not getPedOccupiedVehicle ( player ) then
...
function foundRubbish_func ()
	...
	local player = client
	if not vioGetElementData ( player, "rubbishJob" ) then
		vioSetElementData ( player, "rubbishJob", true )
		vioSetElementData ( player, "streetCleanPoints", vioGetElementData ( player, "streetCleanPoints" ) + 1 )
		setTimer ( vioSetElementData, 1000, 1, player, "rubbishJob", false )
		vioSetElementData ( player, "money", vioGetElementData ( player, "money" ) + 80 )
	end
end
...
function foundSweeperRubbish_func ()
	...
	if not vioGetElementData ( player, "rubbishJob" ) then
		vioSetElementData ( player, "rubbishJob", true )
		vioSetElementData ( player, "streetCleanPoints", vioGetElementData ( player, "streetCleanPoints" ) + 3 )
		setTimer ( vioSetElementData, 3000, 1, player, "rubbishJob", false )
...
function foundTrashmasterRubbish_func ()
	...
	if not vioGetElementData ( player, "rubbishJob" ) then
		vioSetElementData ( player, "rubbishJob", true )
		vioSetElementData ( player, "streetCleanPoints", vioGetElementData ( player, "streetCleanPoints" ) + 5 )
		setTimer ( vioSetElementData, 3000, 1, player, "rubbishJob", false )
...
function trashCheckOccus ( veh )
	...
	if not getVehicleOccupant ( veh ) then
		...
		local eledata = vioGetElementData ( veh, "TrashDriverVehicle" )
...
function createStreetCleanVeh_func ( veh )
	...
	if veh == 574 or veh == 408 then
		...
		setTimer ( trashCheckOccus, 60000, 1, veh )
		vioSetElementData ( veh, "TrashDriverVehicle", getPlayerName(player) )
		...
		vioSetElementData ( player, "isWorking", true )
...
function trashVehDestroy_func ( veh )
	...
	if isElement ( veh ) then
		...
		if model == 574 or model == 408 then
			...
			vioSetElementData ( player, "isWorking", false )
