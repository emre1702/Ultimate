...
function truckerJobIconHit_func ( player )
	...
	if vioGetElementData ( player, "job" ) == "transporteur" and not getPedOccupiedVehicle ( player ) then
...
function startTransporteurRoute_func ( vehicle, time, danger )
	...
	local player = client
	if not vioGetElementData ( player, "transportStarted" ) then
		vioSetElementData ( player, "truckerTrailer", false )
		local value = calcTruckerJobTourValue ( vehicle, time, danger )
		vioSetElementData ( player, "transportStarted", true )
		...
		elseif vehicle == "van" then
			id, z = 440, 15.33
			if vioGetElementData ( player, "carlicense" ) == 0 then
				infobox ( player, "Du brauchst einen\nFuehrerschein!", 5000, 125, 0, 0 )
				return nil
			end
		else
			if vioGetElementData ( player, "lkwlicense" ) == 0 then
		...
		vioSetElementData ( player, "truckerVehicle", vehicle )
		...
		if truck then
			...
			vioSetElementData ( player, "truckerTrailer", trailer )
			...
			if id == 448 then
				...
				vioSetElementData ( player, "truckerBlip", blip )
			else
				-- Van
		...
		vioSetElementData ( vehicle, "TruckerJobShitPlayer", getPlayerName(player) )
		...
		vioSetElementData ( marker, "value", value )
		vioSetElementData ( player, "truckerMarker", marker )
	...
	if dim and hit then
		if isElement ( hit ) then
			if getElementType ( hit ) == "player" then
				...
				local marker = vioGetElementData ( player, "truckerMarker" )
				if source == marker then
					local amount = vioGetElementData ( marker, "value" )
...
function truckerJobVehicleDown ( )
	...
	local veh = vioGetElementData( source, "TruckerJobShitPlayer" )
...
function addTruckerJobErrorEvents ( player, veh )
	...
	local trailer = vioGetElementData ( player, "truckerTrailer" )
...
function truckerJobTrailerDetatch ( truck )
	...
	local player = getVehicleOccupant ( truck )
	if isElement ( player ) then
		if vioGetElementData ( player, "transportStarted" ) then
			cancelTruckerJobForPlayer ( player )
...
function cancelTruckerJobForPlayer ( player )
	...
	local veh = vioGetElementData ( player, "truckerVehicle" )
	local trailer = vioGetElementData ( player, "truckerTrailer" )
	local marker = vioGetElementData ( player, "truckerMarker" )
	local blip = vioGetElementData ( player, "truckerBlip" )
	
	hideallArrows ( player )
	
	vioSetElementData ( player, "transportStarted", false )
